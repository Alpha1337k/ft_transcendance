...
function dragElement(elmnt) {
	...
	function closeDragElement(e) {
		...
		if((e.path[0].id[0] == e.path[0].id[0].toLowerCase()) != gamedata.whiteMove)
		{
		...
		let middleLeft = parseFloat(e.path[0].style.left) + 3.125 - convertPXToVh(pos2);
		let middleTop = parseFloat(e.path[0].style.top) + 3.125 - convertPXToVh(pos1);
		...
		const objToSend = {gameid, newpos: middleTop * 8 + middleLeft, oldpos: oldposY / 6.25 * 8 + oldposX / 6.25};
		oldposX = -1;
		oldposY = -1;
...
function creatediv(posT, posL, img, clr, selectable) {
	...
	if (selectable)
		...
		div.style = 'top: '+ posT +'vh; left: '+ posL +'vh';
...
function loadfen(fen) {
	...
	for (let index = 0; index < fen.length && fen[index] != ' '; index++) {
		...
		{
			board[pos] = new Piece(createpiece(element, pos, (element == element.toLowerCase()) == playerside), element, (element == element.toLowerCase()) == playerside);
			pos++;
		}
	}
}
...
function move_piece(oldpos, newpos, no_remove) {
	if (board[oldpos] == undefined || oldpos == newpos)
		return;
...
async function update_ui() {
	uiInterval = setInterval(() => {
		let p1_cur = gamedata.playerTime[0] - (Date.now() - gamedata.turnStart) / 1000;
		let p2_cur = gamedata.playerTime[1] - (Date.now() - gamedata.turnStart) / 1000;
		if (gamedata.whiteMove && gamedata.firstTurn[1] == false)
			document.getElementById("p2_timer").innerHTML = Math.floor(p2_cur / 60) + ":" + Math.round(p2_cur % 60);
		else if (gamedata.firstTurn[0] == false)
			document.getElementById("p1_timer").innerHTML = Math.floor(p1_cur / 60) + ":" + Math.round(p1_cur % 60);
	}, 1000);
}
