...
function update_movement(deltatime) {
	let movement_speed = 200 * deltatime;
	if (g_keys[2]) movement_speed *= 1.5;
	if (g_keys[0])
		g_player_pos.x =
			g_player_pos.x - movement_speed < 20
				? 20
				: g_player_pos.x - movement_speed;
	if (g_keys[1])
		g_player_pos.x =
			g_player_pos.x + movement_speed > width - g_player_width
...
function calculate_borders() {
	...
	g_borders.push(
		new line(new vec_2(width - 10, 10), new vec_2(width - 10, height - 10))
	);
...
function update_game(deltatime) {
	if (g_ball_exists == false) {
		...
		if (Math.random() > 0.5) g_ball_ang += 180;
		g_ball_dir = new vec_2(
			Math.cos(g_ball_ang * 0.0174532925),
			Math.sin(g_ball_ang * 0.0174532925)
		);
		...
		for (let index = 0; index < g_borders.length; index++) {
			...
			if (intersect(oldpos, g_ball_pos, element.p1, element.p2)) {
				g_ball_speed *= 1.05;
				if (element.p1.y < element.p2.y) g_ball_dir.x = -g_ball_dir.x;
				else g_ball_dir.y = -g_ball_dir.y;
				g_ball_pos.x += g_ball_dir.x * deltatime * g_ball_speed;
				g_ball_pos.y -= g_ball_dir.y * deltatime * g_ball_speed;
				break;
			}
		...
		if (intersect(oldpos, g_ball_pos, g_player_pos, player_end)) {
			...
			g_ball_ang +=
				((g_ball_pos.x - g_player_pos.x) / g_player_width) * 30 - 15;
			...
			if (g_ball_ang > 360) g_ball_ang = g_ball_ang % 360;
			else if (g_ball_ang < 0) g_ball_ang = 360 + g_ball_ang;
			g_ball_pos.x += g_ball_dir.x * deltatime * g_ball_speed;
			g_ball_pos.y -= g_ball_dir.y * deltatime * g_ball_speed;
		}
		if (g_ball_pos.y < 0) {
...
function render() {
	let tl = new vec_2(g_player_pos.x, g_player_pos.y - 5);
