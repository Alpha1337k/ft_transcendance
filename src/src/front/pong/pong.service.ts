...
function ccw(A: { y: number; x: number; }, B: { x: number; y: number; }, C: { y: number; x: number; })
{
...
class Game {
	...
	async run_game() {
		...
		this.inter = setInterval(function () {
			...
			gameRef.update_game();
			gameRef.#serverRef.to(gameRef.roomId).emit('getGameUpdate', new gameUpdate(gameRef));
		}, 1000 / 60);
	}
	...
	update_position(updt : positionUpdate) {
		if (updt.id == 1)
		{
			if (updt.newpos.x < 5)
				this.p1_pos.x = 5;
			else if (updt.newpos.x > 85)
				this.p1_pos.x = 85;
			else
				this.p1_pos.x = updt.newpos.x;
		}
		else if (updt.id == 2)
		{
			if (updt.newpos.x < 5)
	...
	update_game() {
		...
		if (this.ballExists == false)
		{
			...
			if (Math.random() > 0.5)
				this.ballAng += 180;
			this.ballDir = new vec_2(Math.cos(this.ballAng * 0.0174532925), Math.sin(this.ballAng * 0.0174532925));
			this.ballPos = new vec_2(50, 75);
		...
		else
		{
			...
			if (intersect(oldpos, this.ballPos, this.p1_pos, player1_end) ||
