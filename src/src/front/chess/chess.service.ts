...
export class ChessService {
	...
	addToQueue(clientid : Socket, server : Server): void {
		...
		if (this.searching_players.length >= 2)
		{
			...
			this.running_games[this.gameid] = new ChessGame(p1, p2, 300, this.gameid, server);

			
			server.to(p1.id).emit("chessGameFound", {gameId: this.gameid, userId: 1, color: true, game: this.running_games[this.gameid]});
			server.to(p2.id).emit("chessGameFound", {gameId: this.gameid, userId: 2, color: false, game: this.running_games[this.gameid]});
			p1.join(this.gameid);
			p2.join(this.gameid);
...
class ChessGame {
	...
	constructor(p1: Socket, p2: Socket, time: number, id: string, server : Server) {
		this.roomId = id;
		...
		for (let pos = 0, index = 0; index < this.fen.length && this.fen[index] != ' '; index++) {
			const e = this.fen[index];
			if (e == '/')
				continue;
			if (e > '0' && e < '9')
				pos += e.charCodeAt(0) - 48;
			else
			{
				this.#board[pos] = e;
				pos++;
	...
	checkGameOver() {
		...
		else if (this.playerTime[this.whiteMove ? 1 : 0] - (Date.now() - this.turnStart) / 1000 < 0)
			return true;
		return false;
	}
	...
	turnUpdate() {
		...
		{
			this.playerTime[this.whiteMove ? 1 : 0] -= (Date.now() - this.turnStart) / 1000;
			console.log("new time for player", this.whiteMove, this.playerTime[this.whiteMove ? 1 : 0])
		}
	...
	is_allowed(client : Socket, oldpos: number, newpos: number) {
		...
		if (possible)
			possible = this.check_take(newpos, oldpos, color, piece_type);
		if (possible || iscastle)
		{
			...
			if (this.check_checked(color, undefined))
			{
				...
				this.#serverRef.to(client.id).emit("revertMove", {oldpos, newpos, checked: true});
			...
			if (iscastle)
				this.#serverRef.to(this.roomId).emit('moveUpdate', {oldpos, newpos, iscastle, game : this});
			else
				this.#serverRef.to(this.roomId).emit('moveUpdate', {oldpos, newpos, game : this});

		}
		else
		{
			console.log("The move is invalid! ", oldpos, newpos, checked);
			this.#serverRef.to(client.id).emit("revertMove", {oldpos, newpos, checked});
		}
		return (possible);
	}
	...
	update_castles(oldpos : number, piece_type : string, color : boolean)
	{
		if (color == false)
		{
			if (piece_type == 'r' && oldpos == 56)
				this.p2_castle[0] = false;
			else if (piece_type == 'r' && oldpos == 63)
		...
		{
			if (piece_type == 'r' && oldpos == 0)
				this.p1_castle[0] = false;
			else if (piece_type == 'r' && oldpos == 7)
	...
	check_rook(color: boolean, newpos: number, oldpos: number) {
		...
		while (oldpos != newpos && oldpos >= 0 && oldpos <= 63)
		{
			...
			if (this.#board[oldpos] != undefined)
				break;
		}
		if(oldpos == newpos)
			return true;
	...
	check_pawn(color: boolean, newpos: number, oldpos: number) {
		...
		if (color == true && oldpos >= 8 && oldpos <= 15)
			tocheck = oldpos + 16;
	...
	check_queen(color: boolean, newpos: any, oldpos: any) {
		return this.check_bishop(color, newpos, oldpos) || this.check_rook(color, newpos, oldpos);
	}
	...
	check_castle(color: boolean, newpos: number, oldpos: number)
	{
		...
		{
			if (this.p1_castle[0] && newpos == 6 && this.#board[5] == undefined && this.#board[6] == undefined 
				&& !this.check_checked(color, 5) && !this.check_checked(color, 6))
			{
				this.#board[7] = this.#board[5];
				this.#board[7] = undefined;
					return {possible: true, oldpos: 7, newpos: 5};
			}
			if (this.p1_castle[1] && newpos == 2 &&  this.#board[3] == undefined && this.#board[2] == undefined
				&& this.#board[1] == undefined && !this.check_checked(color, 3) && !this.check_checked(color, 2) &&
		...
		{
			if (this.p2_castle[0] && newpos == 62 && this.#board[61] == undefined && this.#board[62] == undefined 
				&& !this.check_checked(color, 61) && !this.check_checked(color, 62))
					return {possible: true, oldpos: 63, newpos: 61};
			if (this.p2_castle[1] && newpos == 58 && this.#board[57] == undefined && this.#board[58] == undefined
				&& this.#board[59] == undefined && !this.check_checked(color, 59) && !this.check_checked(color, 58) &&
	...
	check_king(color: boolean, newpos: number, oldpos: number) {
		if ((newpos >= oldpos - 9 && newpos <= oldpos - 7) || (newpos >= oldpos - 1 && newpos <= oldpos + 1)
	...
	check_knight(color: boolean, newpos: number, oldpos: number) {
		if (newpos == oldpos - 17 || newpos == oldpos - 15 || newpos == oldpos + 17 || newpos == oldpos + 15 ||
			newpos == oldpos + 6 || newpos == oldpos - 6 || newpos == oldpos - 10 || newpos == oldpos + 10
			)
			return true;
		return false;
	}

	check_take(newpos: number, oldpos: number, color: boolean, piece_type: string) {
		if (piece_type == 'p')
		{
			if (newpos % 8 == oldpos % 8 && this.#board[newpos] == undefined)
				return true;
			else if(this.#board[newpos] == undefined || this.#board[newpos] == this.#board[newpos].toLowerCase() == color)
				return false;
			return true;
		}
		else
		{
			if (this.#board[newpos] == undefined)
				return true;	
	...
	check_row(enemies : string[], x : number, rule: (x: any) => boolean, increment: number)
	{
		let start = x;
		for (; rule(x); x += increment)
		{
			for (let i = 0; i < enemies.length; i++)
			{
				...
				{
					console.log("whoops found an enemy!", start, x, enemies[i], increment)
					return true;
				}
			}
	...
	check_checked(color: boolean, kingPos : number | undefined) {
		...
		if (kingPos == undefined)
			i= this.#board.findIndex(x => x == kingValue);
		else
			i = kingPos;
		...
		if (this.#board[i - 17] == enemies[4] || this.#board[i - 15] == enemies[4] || this.#board[i + 17] == enemies[4] ||
			this.#board[i + 15] == enemies[4] || this.#board[i - 6] == enemies[4] || this.#board[i + 6] == enemies[4] ||
			this.#board[i - 10] == enemies[4] || this.#board[i + 10] == enemies[4]
			)
			return true;
		...
		if (this.#board[i - 1] == enemies[3] || this.#board[i + 1] == enemies[3] || this.#board[i - 7] == enemies[3] ||
			this.#board[i - 8] == enemies[3] || this.#board[i - 9] == enemies[3] || this.#board[i + 7] == enemies[3] ||
			this.#board[i + 8] == enemies[3] || this.#board[i + 9] == enemies[3]
			)
			return true;
		...
		if (	this.check_row(enemies, i - 8, x => x >= 0,	-8) || 
